...
summary = "These are guidelines to optimize both the WhatsApp Flows user and developer experience."
source = "https://developers.facebook.com/docs/whatsapp/bestpractices"
lang = "en"
...
### Dont have more than one task per screen
...
### Dont use too many components per screen
...
Including a login screen may be off putting for some users, so try to only use them when absolutely necessary. If you do need one, set the expectations for users so it doesnt come as a surprise.
...
testingdebugging](/docs/whatsapp/flows/guides/testingdebugging)
...
[Flows shouldnt be long](#flows-shouldn-t-be-long)

[Dont have more than one task per screen](#don-t-have-more-than-one-task-per-screen)

[Dont use too many components per screen](#don-t-use-too-many-components-per-screen)

[Build for caching](#build-for-caching)

[Use flows without the endpoint whenever possible](#use-flows-without-the-endpoint-whenever-possible)

[Only use endpoint powered flows, when the live data is required while user completes a flow - e.g. for ticket booking.](#only-use-endpoint-powered-flows--when-the-live-data-is-required-while-user-completes-a-flow---e-g--for-ticket-booking-)

[Technical](#technical)

[Latency](#latency)

[flow\_token Expiration Management](#flow-token-expiration-management)

[Design](#design)

[Call-to-Actions (CTAs)](#call-to-actions--ctas-)
...
[Diverging Flows](#diverging-flows)
...
[Initiation Flow](#initiation-flow)
...
[Options and lists](#options-and-lists)

[Termination Flow](#termination-flow)

[Trust and Support](#trust-and-support)
